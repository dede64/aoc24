    ...
    func solve() {
        ...
        for line in lines {
            if line.contains("|") {
                ...
                rules.append(.init(x: Double(parts[0])!, y: Double(parts[1])!))
            } else {
                let parts = line.split(separator: ",")
                var order: [Double] = []
                for part in parts {
                    order.append(Double(part)!)
                }
                orders.append(order)
            }
        }
        ...
        var result = 0.0
        for order in orders {
            var orderIsCorrect: Bool = true
            
            for i in 0..<order.count {
                guard orderIsCorrect else { break }
                let item = order[i]
                
                // Get all rules with the item
                let rulesWithItem: [CGPoint] = rules.filter { $0.y == item }
                
                for j in (i+1)..<order.count {
                    let nextItem = order[j]
                    let rulesWithItems = rulesWithItem.filter { $0.x == nextItem }
                    if !rulesWithItems.isEmpty {
        ...
        for order in incorrectOrders {
            var fixedOrder: [Double] = []
            
            for item in order {
                let rulesA: [CGPoint] = rules.filter { $0.y == item }
                let itemsBefore: [Double] = rulesA.map(\.x)
                
                var insertIndex = -1
                for item in itemsBefore {
                    if let index = fixedOrder.firstIndex(of: item), index > insertIndex {
                        insertIndex = index
                    }
                }
                fixedOrder.insert(item, at: insertIndex + 1)
            }
            result += fixedOrder[fixedOrder.count/2]
        }
